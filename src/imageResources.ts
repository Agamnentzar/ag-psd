...
function byteAt(value: string, index: number) {
  return (
    (charToNibble(value.charCodeAt(index)) << 4) |
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1028", await left());
      (target as any)._ir1028 = readBytes(reader, await left());
    },
    (writer, target) => {
...
addHandler(
  ...
  async (reader, target, left) => {
    target.xmpMetadata = readUtf8String(reader, await left());
  },
  (writer, target) => {
...
addHandler(
    ...
    if (desc.printProofSetup) {
      if ("Bltn" in desc.printProofSetup) {
        ...
        info.proofSetup = {
          ...
          renderingIntent: Inte.decode(
            desc.printProofSetup.Inte ?? "Inte.Img "
          ),
  ...
  (writer, target) => {
    ...
    if (info.proofSetup && "profile" in info.proofSetup) {
      ...
      desc.printProofSetup = {
        Bltn: info.proofSetup?.builtin
          ? `builtinProof.${info.proofSetup.builtin}`
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1083", await left());
      (target as any)._ir1083 = readBytes(reader, await left());
...
addHandler(
  ...
  async (reader, target) => {
    ...
    target.resolutionInfo = {
      ...
      horizontalResolutionUnit:
        RESOLUTION_UNITS[horizontalResolutionUnit] || ("PPI" as any),
      widthUnit: MEASUREMENT_UNITS[widthUnit] || ("Inches" as any),
      verticalResolution,
      verticalResolutionUnit:
        RESOLUTION_UNITS[verticalResolutionUnit] || ("PPI" as any),
  ...
  (writer, target) => {
    ...
    writeUint16(
      writer,
      Math.max(1, RESOLUTION_UNITS.indexOf(info.horizontalResolutionUnit))
    );
    writeUint16(writer, Math.max(1, MEASUREMENT_UNITS.indexOf(info.widthUnit)));
    writeFixedPoint32(writer, info.verticalResolution || 0);
    writeUint16(
      writer,
      Math.max(1, RESOLUTION_UNITS.indexOf(info.verticalResolutionUnit))
    );
    writeUint16(
      writer,
      Math.max(1, MEASUREMENT_UNITS.indexOf(info.heightUnit))
    );
  }
);
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1077", await left());
      (target as any)._ir1077 = readBytes(reader, await left());
    },
    (writer, target) => {
...
addHandler(
  ...
  async (reader, target) => {
    target.backgroundColor = readColor(reader);
  },
...
addHandler(
  ...
  async (reader, target) => {
    target.globalAngle = readInt32(reader);
  },
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 10000", await left());
      (target as any)._ir10000 = readBytes(reader, await left());
    },
    (writer, target) => {
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1013", await left());
      (target as any)._ir1013 = readBytes(reader, await left());
    },
    (writer, target) => {
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1016", await left());
      (target as any)._ir1016 = readBytes(reader, await left());
    },
    (writer, target) => {
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1092", await left());
      // 16 bytes, seems to be 4 integers
      (target as any)._ir1092 = readBytes(reader, await left());
    },
    (writer, target) => {
...
addHandler(
  ...
  async (reader, target, _, options) => {
    const desc = readVersionAndDescriptor(
      reader
    ) as TimelineInformationDescriptor;
    ...
    target.timelineInformation = {
      ...
      globalTracks: parseTrackList(
        desc.globalTrackList,
    ...
    if (desc.audioClipGroupList?.audioClipGroupList?.length) {
      target.timelineInformation.audioClipGroups =
        desc.audioClipGroupList.audioClipGroupList.map((g) => ({
          id: g.groupID,
          muted: g.muted,
          audioClips: g.audioClipList.map(
            ({ clipID, timeScope, muted, audioLevel, frameReader }) => ({
              id: clipID,
  ...
  async (reader, target) => {
    ...
    if (desc.sheetTimelineOptions) {
      target.sheetDisclosure.sheetTimelineOptions =
        desc.sheetTimelineOptions.map((o) => ({
          sheetID: o.sheetID,
...
function boundsFromBounds(bounds: BoundsDesc): {
  left: number;
  top: number;
  right: number;
  bottom: number;
...
const sliceOrigins: ("userGenerated" | "autoGenerated" | "layer")[] = [
  "autoGenerated",
  "layer",
  "userGenerated",
];
  ...
  async (reader, target) => {
    ...
    if (version === 6) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const horizontalAlignment = clamped(
          sliceAlignments,
          readUint32(reader)
        );
        ...
        const backgroundColorType =
          a + r + g + b === 0 ? "none" : a === 0 ? "matte" : "color";
        slices.push({
          id,
          groupId,
          origin,
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      ...
      LOG_MOCK_HANDLERS && console.log("image resource 1039", await left());
      (target as any)._ir1039 = readBytes(reader, await left());
    },
    (writer, target) => {
...
addHandler(
  ...
  async (reader, target) => {
    target.idsSeedNumber = readUint32(reader);
  },
...
addHandler(
  1036,
  (target) =>
    target.thumbnail !== undefined || target.thumbnailRaw !== undefined,
  async (reader, target, left, options) => {
    ...
    if (format !== 1 || bitsPerPixel !== 24 || planes !== 1) {
      options.logMissingFeatures &&
        console.log(
          `Invalid thumbnail data (format: ${format}, bitsPerPixel: ${bitsPerPixel}, planes: ${planes})`
        );
      skipBytes(reader, await left());
      return;
    }

    const size = await left();
  ...
  (writer, target) => {
    ...
    if (target.thumbnailRaw) {
      ...
      const dataUrl = target
        .thumbnail!.toDataURL("image/jpeg", 1)
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1058", await left());
      (target as any)._ir1058 = readBytes(reader, await left());
    },
    (writer, target) => {
...
addHandler(
  ...
  async (reader, target, left) => {
    target.imageReadyVariables = readUtf8String(reader, await left());
  },
  (writer, target) => {
...
addHandler(
  ...
  async (reader, target, left) => {
    target.imageReadyDataSets = readUtf8String(reader, await left());
  },
  (writer, target) => {
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 1025", await left());
      (target as any)._ir1025 = readBytes(reader, await left());
    },
    (writer, target) => {
...
addHandler(
  ...
  async (reader, target, left, { logMissingFeatures, logDevFeatures }) => {
        ...
        while ((await left()) > 0) {
          ...
          await readSection(reader, 1, async (left) => {
            if (key === "AnDs") {
              ...
              const bytes = readBytes(reader, await left());
              logDevFeatures && console.log("#4000 Roll", bytes);
            } else {
              logMissingFeatures &&
                console.log("Unhandled subsection in #4000", key);
            }
          });
        }
      });
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left, { logMissingFeatures, logDevFeatures }) => {
      if (MOCK_HANDLERS) {
        LOG_MOCK_HANDLERS && console.log("image resource 4001", await left());
        (target as any)._ir4001 = readBytes(reader, await left());
        return;
      }
...
MOCK_HANDLERS &&
  addHandler(
    ...
    async (reader, target, left) => {
      LOG_MOCK_HANDLERS && console.log("image resource 4002", await left());
      (target as any)._ir4002 = readBytes(reader, await left());
    },
    (writer, target) => {
