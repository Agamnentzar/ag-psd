...
import {setLogErrors} from 'ag-psd/dist/descriptor';
...
import {Psd, PsdExtended, ReadOptions, readPsdExtended} from '../index';
...
export function readPsdFromFile(fileName: string, options?: ReadOptions): PsdExtended {
	const buffer = fs.readFileSync(fileName);
	const reader = createReaderFromBuffer(buffer);
	return readPsdExtended(reader, options) as PsdExtended;
}
