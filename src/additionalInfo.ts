...
function readLength64(reader: PsdReader) {
  if (readUint32(reader))
    throw new Error(
      `Resource size above 4 GB limit at ${reader.offset.toString(16)}`
    );
...
addHandler(
  ...
  async (reader, target, leftBytes) => {
    ...
    skipBytes(reader, await leftBytes());
  },
  (writer, target) => {
    ...
    const textDescriptor: TextDescriptor = {
      ...
      ...(text.boundingBox
        ? { boundingBox: boundsToDescBounds(text.boundingBox) }
        : {}),
...
export function readBezierKnot(
  reader: PsdReader,
  width: number,
  height: number
) {
...
export const booleanOperations: BooleanOperation[] = [
  "exclude",
  "combine",
  "subtract",
  "intersect",
...
export function readVectorMask(
  reader: PsdReader,
...
addHandler(
  "vmsk",
  hasKey("vectorMask"),
  async (reader, target, left, { width, height }) => {
    ...
    readVectorMask(reader, vectorMask, width, height, await left());
...
addHandler(
  ...
  async (reader, target, left, _, options) => {
...
addHandler(
  ...
  hasKey("nameSource"),
  async (reader, target) => {
    target.nameSource = readSignature(reader);
  },
...
addHandler(
    ...
    for (let i = 0; i < count; i++) {
      ...
      await readSection(reader, 1, async (left) => {
        if (key === "cust") {
          ...
          if (desc.trackList) {
            timeline.tracks = parseTrackList(
              desc.trackList,
              !!options.logMissingFeatures
            );
          }
  ...
  (writer, target, _, options) => {
    const { animationFrames, animationFrameFlags, timestamp, timeline, comps } =
      target;
...
addHandler(
  ...
  async (reader, target, left) => {
    ...
    target.vectorStroke = {
      ...
      lineJoinType: strokeStyleLineJoinType.decode(
        desc.strokeStyleLineJoinType
      ),
      lineAlignment: strokeStyleLineAlignment.decode(
        desc.strokeStyleLineAlignment
      ),
  ...
  (writer, target) => {
    ...
    const desc: StrokeDescriptor = {
      ...
      strokeStyleLineJoinType: strokeStyleLineJoinType.encode(
        stroke.lineJoinType
      ),
      strokeStyleLineAlignment: strokeStyleLineAlignment.encode(
        stroke.lineAlignment
      ),
      ...
      strokeStyleContent: serializeVectorContent(
        stroke.content || { type: "color", color: { r: 0, g: 0, b: 0 } }
      ).descriptor,
...
addHandler(
  ...
  async (reader, target) => {
    target.usingAlignedRendering = !!readUint32(reader);
  },
  (writer, target) => writeUint32(writer, target.usingAlignedRendering ? 1 : 0)
);

const placedLayerTypes: PlacedLayerType[] = [
  "unknown",
  "vector",
  "raster",
  "image stack",
...
function parseWarp(warp: WarpDescriptor & QuiltWarpDescriptor): Warp {
  ...
  if (envelopeWarp) {
    ...
    const xs =
      envelopeWarp.meshPoints.find((i) => i.type === "Hrzn")?.values || [];
    const ys =
      envelopeWarp.meshPoints.find((i) => i.type === "Vrtc")?.values || [];
    ...
    if (envelopeWarp.quiltSliceX || envelopeWarp.quiltSliceY) {
      result.customEnvelopeWarp.quiltSliceX =
        envelopeWarp.quiltSliceX?.[0]?.values || [];
      result.customEnvelopeWarp.quiltSliceY =
        envelopeWarp.quiltSliceY?.[0]?.values || [];
    }
  }
...
function isQuiltWarp(warp: Warp) {
  return (
    warp.deformNumCols != null ||
    warp.deformNumRows != null ||
    warp.customEnvelopeWarp?.quiltSliceX ||
    warp.customEnvelopeWarp?.quiltSliceY
  );
}
...
function encodeWarp(warp: Warp): WarpDescriptor {
  ...
  const desc: WarpDescriptor = {
    ...
    bounds: {
      "Top ": unitsValue(
        (bounds && bounds.top) || { units: "Pixels", value: 0 },
        "bounds.top"
      ),
      Left: unitsValue(
        (bounds && bounds.left) || { units: "Pixels", value: 0 },
        "bounds.left"
      ),
      Btom: unitsValue(
        (bounds && bounds.bottom) || { units: "Pixels", value: 0 },
        "bounds.bottom"
      ),
      Rght: unitsValue(
        (bounds && bounds.right) || { units: "Pixels", value: 0 },
        "bounds.right"
      ),
    },
...
addHandler(
  ...
  async (reader, target, left) => {
    ...
    const warp: WarpDescriptor & QuiltWarpDescriptor =
...
function uint8ToFloat32(array: Uint8Array) {
  return new Float32Array(
    array.buffer.slice(array.byteOffset),
    0,
    array.byteLength / 4
  );
}
...
function parseFilterFXItem(f: SoLdDescriptorFilterItem): Filter {
  ...
  switch (f.filterID) {
    case 1098281575:
      return { ...base, type: "average" };
    case 1114403360:
      return { ...base, type: "blur" };
    case 1114403405:
      return { ...base, type: "blur more" };
    case 697:
      return {
        ...base,
        type: "box blur",
    ...
    case 1180922912:
      return { ...base, type: "facet" };
    case 1181902701:
      return { ...base, type: "fragment" };
    case 1299870830:
      return {
        ...base,
        type: "mezzotint",
    ...
    case 1399353968:
      return { ...base, type: "sharpen" };
    case 1399353925:
      return { ...base, type: "sharpen edges" };
    case 1399353933:
      return { ...base, type: "sharpen more" };
    case 698:
      return {
        ...base,
        type: "smart sharpen",
    ...
    case 1181639749:
      return { ...base, type: "find edges" };
    case 1399616122:
      return { ...base, type: "solarize" };
    case 1416393504:
      return {
        ...base,
        type: "tiles",
...
function serializeFilterFXItem(f: Filter): SoLdDescriptorFilterItem {
  ...
  switch (f.type) {
    case "average":
      return { ...base, filterID: 1098281575 };
    case "blur":
      return { ...base, filterID: 1114403360 };
    case "blur more":
      return { ...base, filterID: 1114403405 };
    case "box blur":
      return {
        ...base,
        Fltr: {
    ...
    case "shear":
      return {
        ...
        Fltr: {
          _name: "Shear",
          _classID: "Shr ",
          ShrP: f.filter.shearPoints.map((p) => ({
            _name: "",
            _classID: "Pnt ",
            Hrzn: p.x,
            Vrtc: p.y,
    ...
    case "despeckle":
      return { ...base, filterID: 1148416099 };
    ...
    case "facet":
      return { ...base, filterID: 1180922912 };
    case "fragment":
      return { ...base, filterID: 1181902701 };
    case "mezzotint":
      return {
        ...base,
        Fltr: {
    ...
    case "sharpen":
      return { ...base, filterID: 1399353968 };
    case "sharpen edges":
      return { ...base, filterID: 1399353925 };
    case "sharpen more":
      return { ...base, filterID: 1399353933 };
    case "smart sharpen":
      return {
        ...base,
        Fltr: {
    ...
    case "find edges":
      return { ...base, filterID: 1181639749 };
    case "solarize":
      return { ...base, filterID: 1399616122 };
    case "tiles":
      return {
        ...base,
        Fltr: {
    ...
    case "ntsc colors":
      return { ...base, filterID: 1314149187 };
    case "custom":
      return {
        ...base,
        Fltr: {
      ...
      return {
        ...
        Fltr: {
          ...
          puppetShapeList: f.filter.puppetShapeList.map((p) => ({
            ...
            VrsM: 1, // TODO: ???
            VrsN: 0, // TODO: ???
            originalVertexArray: toUint8(
              new Float32Array(pointsToArray(p.originalVertexArray))
            ),
            deformedVertexArray: toUint8(
              new Float32Array(pointsToArray(p.deformedVertexArray))
            ),
    ...
    case "oil paint plugin": {
      ...
      for (let i = 0; i < f.filter.parameters.length; i++) {
        ...
        const suffix = `${fromAtoZ[Math.floor(i / fromAtoZ.length)]}${
    ...
    default:
      throw new Error(`Unknow filter type: ${(f as any).type}`);
  }
}
...
function getWarpFromPlacedLayer(placed: PlacedLayer): Warp {
  ...
  if (!placed.width || !placed.height)
    throw new Error(
      "You must provide width and height of the linked image in placedLayer"
    );
...
addHandler(
  ...
  async (reader, target, left) => {
    ...
    if (
      desc.nonAffineTransform &&
      desc.nonAffineTransform.some((x, i) => x !== desc.Trnf[i])
    ) {
    ...
    skipBytes(reader, await left()); // HACK
  },
  (writer, target) => {
    ...
    const desc: SoLdDescriptor = {
      ...
      Rslt: placed.resolution
        ? unitsValue(placed.resolution, "resolution")
...
addHandler(
  "Lr16",
  () => false,
  async (reader, _target, _left, psd, options) => {
    await readLayerInfo(reader, psd, options);
  },
  (_writer, _target) => {}
);
...
if (MOCK_HANDLERS) {
  addHandler(
    ...
    async (reader, target, left) => {
      ...
      reader;
      target;
      console.log("vowv", { value }, await left());
    },
    (_writer, _target) => {
      // TODO: write
    }
...
if (MOCK_HANDLERS) {
  addHandler(
    ...
    async (reader, target, left) => {
      ...
      (target as any)._Patt = readBytes(reader, await left());
    },
...
addHandler(
  "lnk2",
  (target: any) =>
    !!(target as Psd).linkedFiles && (target as Psd).linkedFiles!.length > 0,
  async (reader, target, left, _, options) => {
    ...
    while ((await left()) > 8) {
      ...
      const fileOpenDescriptor = hasFileOpenDescriptor
        ? (readVersionAndDescriptor(reader) as FileOpenDescriptor)
        : undefined;
      const linkedFileDescriptor =
        type === "liFE" ? readVersionAndDescriptor(reader) : undefined;
  ...
  (writer, target) => {
    ...
    for (const file of psd.linkedFiles!) {
      ...
      writeSignature(
        writer,
        file.type ? `${file.type}    `.substring(0, 4) : "    "
      );
      writeSignature(
        writer,
        file.creator ? `${file.creator}    `.substring(0, 4) : "\0\0\0\0"
      );
...
function adjustmentType(type: string) {
  return (target: LayerAdditionalInfo) =>
    !!target.adjustment && target.adjustment.type === type;
}
...
function writeLevelsChannel(
  writer: PsdWriter,
  channel: LevelsAdjustmentChannel
) {
...
function writeCurveChannel(
  writer: PsdWriter,
  channel: CurvesAdjustmentChannel
) {
...
function writeHueChannel(
  writer: PsdWriter,
  channel: HueSaturationAdjustmentChannel | undefined
) {
...
function writeColorBalance(
  writer: PsdWriter,
  value: Partial<ColorBalanceValues>
) {
...
function writeMixrChannel(
  writer: PsdWriter,
  channel: ChannelMixerChannel | undefined
) {
...
const colorLookupType = createEnum<
  "3dlut" | "abstractProfile" | "deviceLinkProfile"
>("colorLookupType", "3DLUT", {
  "3dlut": "3DLUT",
  abstractProfile: "abstractProfile",
  deviceLinkProfile: "deviceLinkProfile",
});

const LUTFormatType = createEnum<"look" | "cube" | "3dl">(
  "LUTFormatType",
  "look",
  {
    look: "LUTFormatLOOK",
...
addHandler(
  ...
  async (reader, target, left) => {
    ...
    info.colorModel = (grdmColorModels[readUint16(reader)] || "rgb") as
      | "rgb"
      | "hsb"
      | "lab";
  ...
  (writer, target) => {
    ...
    for (let i = 0; i < 4; i++)
      writeUint16(
        writer,
        Math.round(((info.min && info.min[i]) || 0) * 0x8000)
      );

    for (let i = 0; i < 4; i++)
      writeUint16(
        writer,
        Math.round(((info.max && info.max[i]) || 0) * 0x8000)
      );
...
addHandler(
  ...
  (target) => {
    ...
    return (
      (a.type === "brightness/contrast" && !a.useLegacy) ||
      ((a.type === "levels" ||
        a.type === "curves" ||
        a.type === "exposure" ||
  ...
  async (reader, target, left) => {
    const desc = readVersionAndDescriptor(reader) as
      ...
      | PresetDescriptor
      | CurvesPresetDescriptor
      | MixerPresetDescriptor;
...
addHandler(
  ...
  hasKey("engineData"),
  async (reader, target, left) => {
    const data = readBytes(reader, await left());
...
addHandler(
  ...
  async (reader, target, leftBytes) => {
    ...
    while (reader.offset < end) {
      ...
      const channels: (
        | { compressionMode: number; data: Uint8Array }
        | undefined
      )[] = [];
      ...
      if ((await leftBytes()) && readUint8(reader)) {
        const compressionMode = readUint16(reader);
        const data = readBytes(reader, await leftBytes());
        target.filterEffectsMasks[target.filterEffectsMasks.length - 1].extra =
          { compressionMode, data };
      }
    }
  },
  (writer, target) => {
...
addHandler(
  ...
  async (reader, target, left) => {
    ...
    (target as Psd).artboards = {
      ...
      docDefaultNewArtboardBackgroundColor: parseColor(
        desc.docDefaultNewArtboardBackgroundColor
      ),
      docDefaultNewArtboardBackgroundType:
        desc.docDefaultNewArtboardBackgroundType,
    };
  ...
  (writer, target) => {
    ...
    const desc: ArtdDescriptor = {
      ...
      docDefaultNewArtboardBackgroundColor: serializeColor(
        artb.docDefaultNewArtboardBackgroundColor
      ),
      docDefaultNewArtboardBackgroundType:
        artb.docDefaultNewArtboardBackgroundType ?? 1,
    };
...
addHandler(
  ...
  async (reader, target, left, _, options) => {
    ...
    const desc: Lfx2Descriptor & LmfxDescriptor =
      readVersionAndDescriptor(reader);
...
addHandler(
  ...
  async (reader, target, left) => {
    ...
    if (desc.enableCompCoreGPU)
      target.compositorUsed.enableCompCoreGPU = enumValue(
        desc.enableCompCoreGPU
      );
    if (desc.compCoreSupport)
      target.compositorUsed.compCoreSupport = enumValue(desc.compCoreSupport);
    if (desc.compCoreGPUSupport)
      target.compositorUsed.compCoreGPUSupport = enumValue(
        desc.compCoreGPUSupport
      );
...
addHandler(
  ...
  (writer, target) => {
    ...
    if (MOCK_HANDLERS)
      writeVersionAndDescriptor(writer, "", "null", (target as any)._extn);
  }
);
